...
def featurize_as_graph(protein, num_rbf  = 16, device = "cuda"):
    ...
    with torch.no_grad():
        ...
        X_ca = coords[:, 1]
        # print(f"alpha carbon coordinates {X_ca.shape}")
        #X_ca = coords
        edge_index = torch_cluster.radius_graph(X_ca, r = 5.0) # 5 angstrom 
        #edge_index = torch_cluster.knn_graph(X_ca, k=30)
        
        pos_embeddings = positional_embeddings(edge_index) 
        E_vectors = X_ca[edge_index[0]] - X_ca[edge_index[1]] # displacement vectors b/w each CA 
        rbf = _rbf(E_vectors.norm(dim=-1), D_count=num_rbf, device = device) # Radial basis function 
        
        dihedrals = get_dihedrals(coords)              
        orientations = get_orientations(X_ca)
        sidechains = get_sidechains(coords)
        # print(f"orientations {orientations}")
        # print(f"sidechains {sidechains}")        
        node_s = dihedrals
        node_v = torch.cat([orientations, sidechains.unsqueeze(-2)], dim=-2) # CA forward, CA backward, Sidechain
        
        edge_s = torch.cat([rbf, pos_embeddings], dim=-1) # this is redundant with what's in my network. 
        edge_v = _normalize(E_vectors).unsqueeze(-2) # this is redundant with what's in my network. 
...
def get_orientations(X):
    ...
    # print("forward padded", forward)
    backward = F.pad(backward, [0, 0, 1, 0])
    # print("backward padded", backward)
