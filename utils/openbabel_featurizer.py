    ...
    def get_features(self, molecule, molcode=None):
        ...
        for i, atom in enumerate(molecule):
            ...
            # ignore hydrogens and dummy atoms (they have atomicnum set to 0)
                ...
                # print("1",self.encode_num(atom.atomicnum))
                # print("2",[atom.__getattribute__(prop) for prop in self.NAMED_PROPS])
                # print("3", [func(atom) for func in self.CALLABLES])
        ...
        # print("NAMED PROPS", self.NAMED_PROPS)
        # print("CALLABLES", self.CALLABLES)
        ...
        # print("4",self.find_smarts(molecule)[heavy_atoms])
        if np.isnan(features).any():
            raise RuntimeError('Got NaN when calculating features')

        return coords, features, h_num, self.FEATURE_NAMES,
